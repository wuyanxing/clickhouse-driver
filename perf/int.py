...
data = [(x, x, x, x, x, x, x, x, x, x) for x in range(ITEMS)]
...
%timeit [int(x) for line in requests.get(URL, params={'query': 'SELECT * FROM test LIMIT {} FORMAT TabSeparated'.format(ITEMS)}).text.split('\n') for x in line.split('\t') if line]
